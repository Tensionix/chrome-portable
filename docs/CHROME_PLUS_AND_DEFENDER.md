...
Именно он делает сборку портативной: рядом с `chrome.exe` кладётся его
`version.dll` — «прокси-DLL». `chrome.exe` импортирует `VERSION.dll`, этого имени
нет в списке `KnownDLLs`, поэтому браузер грузит соседнюю обёртку вместо
...
Неподписанная DLL с именем системной, положенная вплотную к `chrome.exe`, чтобы
...
`chrome.exe` as a proxy DLL. `chrome.exe` imports `VERSION.dll`, the name is not
in `KnownDLLs`, so the browser loads the neighbouring wrapper instead of the
...
An unsigned DLL bearing a system name, placed right next to `chrome.exe` so it is
loaded in place of the real one, is a textbook "DLL hijacking" shape to an
